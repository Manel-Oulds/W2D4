...
def all_vowel_pairs(words)
  ...
  words.each_with_index do |c,i|
    (i+1...words.length).each do |j|
      pairs << c+' ' +words[j] if v.all?{|x| (c+words[j]).include?(x)}
    end
  end
  return pairs
...
class String
  def substrings(length = nil)
    ...
    if !length
      ...
      return (sub.select{|x| x.length == length})
    end
  end
end 
...
class String
  def caesar_cipher(num)
    alphabet = ('a'..'z').to_a
    result = ""
    self.each_char.with_index do |car,i|
      result+= alphabet[(alphabet.index(car)+num)%26]
    end
    result
  end
end